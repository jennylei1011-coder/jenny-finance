...
def get_mineru_api_service():
    configured_url = get_config_value("MINERU_API_URL").rstrip("/")
    if configured_url:
...
def read_text_with_mineru_api(file_bytes, filename, api_url, profile):
    token = get_config_value("MINERU_API_TOKEN")
    headers = {"Authorization": f"Bearer {token}"} if token else {}
    files = {
        "files": (
            clean_name(filename, 120),
    ...
    if suffix == ".pdf":
        data["start_page_id"] = "0"
        data["end_page_id"] = str(max(0, profile["max_pdf_pages"] - 1))
    ...
    lines = collect_mineru_text(result)
    text = "\n".join(line for line in lines if line)
    if text.strip():
        return text
    raise RuntimeError("MINERU API 未返回可解析文本")
...
def read_text_with_mineru(file_bytes, filename, profile):
    ...
    configured_api_url = get_config_value("MINERU_API_URL").rstrip("/")
    if configured_api_url:
        return read_text_with_mineru_api(file_bytes, filename, configured_api_url, profile)

    mineru_command = shutil.which("mineru")
    if not mineru_command:
        raise RuntimeError("未找到 MINERU 命令或 MINERU_API_URL，已改用本地备用识别")
